...
export function InsurerLogo({ insurer, size = 44 }: { insurer: Insurer; size?: number }) {
  return (
    ...
    >
      ...
      <span
        className="relative font-display font-semibold tracking-[-0.03em]"
    ...
    >
      <div className="mx-auto max-w-[1800px]">
        ...
        <Reveal className="mb-8 flex flex-col gap-3 border border-warn/35 bg-warn/[0.09] px-5 py-4 sm:flex-row sm:items-center sm:gap-5 sm:px-7">
          ...
          <p className="text-[0.88rem] leading-[1.6] text-navy/68">
            ...
            <Reveal key={insurer.id} as="li" delay={i * 55}>
              ...
              >
                <span className="flex w-full items-start justify-between gap-3">
                  ...
                  {!insurer.enabled && (
                    <span className="micro-sm text-navy/35">Disabled</span>
                  )}
                </span>

                <span className="flex-1">
                  <span className="block font-display text-[1.02rem] leading-tight font-semibold tracking-[-0.02em] text-navy">
                    {insurer.name}
                  </span>
                  <span className="mt-1.5 block micro-sm text-navy/40">{insurer.plan}</span>
                </span>
                ...
                <span
                  className="h-px w-0 bg-teal transition-[width] duration-600 ease-[var(--ease-out-expo)] group-hover:w-full"
                  aria-hidden="true"
                />
              </button>
            </Reveal>
        ...
        <Reveal className="mt-8 max-w-4xl">
          <p className="micro-sm leading-[1.9] text-navy/40">{node.footnote}</p>
        </Reveal>
      </div>
          ...
          <div className="animate-rise relative w-full max-w-lg overflow-hidden border border-white/12 bg-navy p-7 text-paper shadow-[0_40px_120px_-40px_rgba(0,0,0,0.9)] sm:p-9">
            <div className="flex items-start gap-4">
              ...
              <button
                type="button"
                onClick={() => setActive(null)}
                className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-white/15 text-paper/60 transition-colors hover:border-cyan/50 hover:text-cyan"
                aria-label="Close"
              >
            ...
            <ul className="mt-5 space-y-2.5">
              {[
                ...
                <li key={line} className="flex gap-3.5 text-[0.88rem] leading-[1.55] text-paper/62">
                  <span className="micro-sm mt-0.5 shrink-0 text-cyan/60">
