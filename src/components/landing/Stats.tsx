    ...
    >
      <div className="mx-auto max-w-[1800px]">
        ...
        <ul className="border-t border-navy/12">
          {STATISTICS.map((stat, i) => (
            <li key={stat.source + stat.value}>
              <Reveal
                delay={i * 90}
                className="group grid gap-5 border-b border-navy/12 py-9 transition-colors duration-500 hover:bg-navy/[0.03] sm:py-11 lg:grid-cols-[minmax(0,22rem)_1fr_minmax(0,20rem)] lg:items-center lg:gap-10"
              >
                <div className="flex items-baseline gap-4">
                  <span className="micro-sm w-6 shrink-0 text-teal">
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  <span className="font-display text-[clamp(3rem,8vw,5.5rem)] leading-[0.82] font-semibold tracking-[-0.05em] text-navy tabular-nums">
                    <Counter value={stat.value} />
                  </span>
                </div>

                <div>
                  <p className="micro-sm mb-2.5 text-navy/40">{stat.unit}</p>
                  <p className="max-w-[46ch] text-[1rem] leading-[1.55] text-navy/78 sm:text-[1.15rem]">
                    {stat.claim}
                  </p>
                </div>

                <div className="lg:text-right">
                  <p className="micro-sm leading-[1.7] text-navy/45">{stat.source}</p>
                  <p className="mt-1.5 inline-flex items-center gap-2 rounded-full border border-navy/15 px-2.5 py-1 micro-sm text-navy/55">
        ...
        <Reveal className="mt-8 flex max-w-4xl gap-3.5">
          <svg viewBox="0 0 16 16" className="mt-0.5 h-3.5 w-3.5 shrink-0 text-navy/35" fill="none" aria-hidden="true">
            <circle cx="8" cy="8" r="7" stroke="currentColor" strokeWidth="1.2" />
            <path d="M8 7.2v4.2M8 4.8v.9" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
          </svg>
          <p className="micro-sm leading-[1.9] text-navy/40">{STATISTICS_NOTE}</p>
        </Reveal>
      </div>
    </section>
  );
