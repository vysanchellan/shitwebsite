        ...
        <div className="grid gap-12 lg:grid-cols-[1.4fr_1fr] lg:gap-20">
          <ul className="border-t border-navy/12">
            {FAQ.map((item, i) => {
              const isOpen = open === i;
              return (
                <Reveal key={item.q} as="li" delay={i * 50} className="border-b border-navy/12">
                  <h3>
                    <button
                      type="button"
                    ...
                    >
                      <span className="micro-sm mt-2 w-6 shrink-0 text-teal">
                        {String(i + 1).padStart(2, '0')}
                      </span>

                      <span className="flex-1 font-display text-[1.15rem] leading-[1.3] font-semibold tracking-[-0.025em] text-navy transition-colors duration-400 group-hover:text-teal sm:text-[1.45rem]">
                        {item.q}
                      </span>

                      <span
                        className="relative mt-2 grid h-6 w-6 shrink-0 place-items-center rounded-full border border-navy/20 transition-colors duration-400 group-hover:border-teal"
                        aria-hidden="true"
                      >
                        <span className="absolute h-px w-2.5 bg-navy/70" />
                        <span
                          className={`absolute h-2.5 w-px bg-navy/70 transition-transform duration-500 ease-[var(--ease-out-expo)] ${
                  ...
                  >
                    <div className="overflow-hidden">
                      <p className="pb-7 pl-11 text-[0.95rem] leading-[1.7] text-navy/65 sm:max-w-[60ch] sm:text-[1.02rem]">
                        {item.a}
                      </p>
                    </div>
                  </div>
          ...
          <Reveal delay={120} id="contact" className="lg:pt-2">
            <div className="sticky top-28 border border-navy/12 bg-white p-7 sm:p-9">
              ...
              {contact.body.map((p) => (
                <p key={p} className="mt-4 text-[0.92rem] leading-[1.65] text-navy/62">
                  {p}
                </p>
              ))}
              ...
              >
                <span>
                  ...
                  <span className="mt-1.5 block font-display text-[1.02rem] font-semibold tracking-[-0.02em]">
                    {SUPPORT_EMAIL}
                  </span>
                </span>
                <svg
              ...
              <p className="mt-5 micro-sm leading-[1.85] text-navy/38">
